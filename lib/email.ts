...
/**
 * A one-off notice that we rewrote a household's allergy entry.
 *
 * Sent by email rather than left in the app, because the households this
 * affects are exactly the ones who don't come back — an in-app banner would
 * have sat unread next to the very entry it was warning about. It says what
 * they wrote, what it now says, that nothing was removed, and that the change
 * is theirs to undo.
 */
export async function sendRestrictionCorrectionEmail(
  settings: { resendApiKey: string; fromEmail: string; fromName: string },
  toEmail: string,
  data: { from: string[]; to: string[]; settingsUrl: string },
) {
  const resend = new Resend(settings.resendApiKey);

  const row = (label: string, value: string, strong = false) =>
    `<tr>
       <td style="padding:6px 14px 6px 0;color:#9A8B7B;font-size:13px;white-space:nowrap;vertical-align:top">${label}</td>
       <td style="padding:6px 0;color:#3D2714;font-size:15px${strong ? ';font-weight:bold' : ''}">${value}</td>
     </tr>`;
  ...
  <div style="background:#FBF7F0;padding:28px 0;font-family:Georgia,'Times New Roman',serif">
    <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:18px;padding:40px">
      <div style="font-size:23px;color:#3D2714;margin-bottom:22px;line-height:1.3">
        We've corrected how your allergies were written down
      </div>

      <p style="font-size:15px;color:#6B5B4B;line-height:1.7;margin:0 0 16px">
        Fornello checks every recipe against the allergies you gave us. We found a
        bug in that check: it matches on ingredient names, and it wasn't recognising
        the way your entry was written. Your menus may not have been screened as
        thoroughly as they should have been.
      </p>
      <p style="font-size:15px;color:#6B5B4B;line-height:1.7;margin:0 0 22px">
        The bug is fixed, and we've rewritten your entry so every check sees it:
      </p>

      <table style="width:100%;background:#F7F4EE;border-radius:12px;padding:16px 20px;margin:0 0 22px">
        ${row('You wrote', data.from.join(' · '))}
        ${row('Now stored as', data.to.join(' · '), true)}
      </table>

      <p style="font-size:15px;color:#6B5B4B;line-height:1.7;margin:0 0 22px">
        Nothing was removed — we only made your entry more specific. If this isn't
        a fair reading of what you meant, please change it. Your entry is what we
        go by, and you can put back exactly what you wrote.
      </p>

      <div style="text-align:center;margin:0 0 26px">
        <a href="${data.settingsUrl}" style="display:inline-block;background:#4A7859;color:#fff;text-decoration:none;padding:13px 30px;border-radius:999px;font-size:14px">
          Check my allergies
        </a>
      </div>

      <div style="background:#FBF3F1;border-radius:12px;padding:16px 20px">
        <p style="margin:0;font-size:13px;color:#8B2E22;line-height:1.65">
          Recipes are AI-generated and avoid the ingredients you've listed, but you
          must also check every ingredient and label before you cook. An allergen can
          hide inside a stock, a sauce or a spice blend without ever being named, and
          the same product varies from brand to brand. If someone in your family has a
          severe allergy, treat every recipe as a starting point to check rather than
          a list to trust.
        </p>
      </div>

      <p style="font-size:13px;color:#9A8B7B;line-height:1.6;margin:22px 0 0">
        Sorry for the error — and thank you for trusting us with something that
        matters. Just reply to this email if anything looks wrong.
      </p>
    </div>
  </div>`;
  ...
  const { error } = await resend.emails.send({
    ...
    replyTo: REPLY_TO_EMAIL,
    to: [toEmail],
    subject: 'We corrected how your allergies were written down',
    html,
  });
  if (error) throw new Error(error.message);
}
