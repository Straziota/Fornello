...
export default function AddRecipePage({ slug }: { slug: string }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      router.push(familyKitchenHref(slug));
    } catch (e: any) {
      setToast({ msg: e.message, type: 'error' });
      setSaving(false);
    }
  ...
  return (
    <>
      ...
      <Link href={familyKitchenHref(slug)} className="inline-block text-xs uppercase tracking-widest mb-6 transition-opacity hover:opacity-70"
